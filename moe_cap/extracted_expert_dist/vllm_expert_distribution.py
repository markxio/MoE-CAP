...
_config = {
    "enabled": os.environ.get("VLLM_EXPERT_DIST_ENABLED", "true").lower() == "true",
    "mode": os.environ.get("VLLM_EXPERT_DIST_MODE", "stat"),
    "output_dir": os.environ.get("VLLM_EXPERT_DIST_OUTPUT_DIR", "./outputs"),
    "verbose": os.environ.get("VLLM_EXPERT_DIST_VERBOSE", "false").lower() == "true",
...
def enable_expert_distribution(
    ...
    logger.info(f"Expert distribution enabled: mode={mode}, enabled={enabled}, auto_record={auto_record}")
...
def set_mode(mode: str):
    ...
    if mode not in ["stat", "per_token", "per_pass"]:
        raise ValueError(f"Invalid mode: {mode}. Must be one of: stat, per_token, per_pass")
    _config["mode"] = mode
...
def _configure_llm_recording(llm):
    ...
    try:
        # Configure recording
        llm.collective_rpc("configure_expert_distribution_recording", args=(_config["mode"], _config["verbose"]))
        _configured_llms.add(id(llm))
        logger.debug(f"Configured expert distribution recording for LLM instance (mode={_config['mode']})")
    except Exception as e:
        logger.warning(f"Failed to configure expert distribution recording: {e}")


def _extract_expert_stats(all_data: List[Dict], mode: str) -> Optional[Dict[str, Any]]:
    ...
    if mode == "stat":
        ...
        if "aggregated_expert_counts" in data:
            ...
            if counts and len(counts) > 0 and len(counts[0]) > 0:
                ...
                expert_totals = counts_tensor.sum(dim=0) if counts_tensor.ndim > 1 else counts_tensor
                
                active_experts = (expert_totals > 0).sum().item() if isinstance(expert_totals, torch.Tensor) else sum(1 for c in expert_totals if c > 0)
                total_experts = len(expert_totals) if isinstance(expert_totals, torch.Tensor) else len(expert_totals)
                
                return {
                    "mode": "stat",
                    "active_experts": active_experts,
                    "total_experts": total_experts,
                    "tokens_per_expert": expert_totals.tolist() if isinstance(expert_totals, torch.Tensor) else expert_totals,
        ...
        if all_records:
            if mode == "per_token":
                return {
                    ...
                    "sample_routing": all_records[0].get("topk_ids") if all_records else None,
                }
            else:  # per_pass
                ...
                return {
                    "mode": "per_pass",
                    "num_records": len(all_records),
                    "avg_experts_activated": sum(avg_activated) / len(avg_activated) if avg_activated else None,
                    "forward_pass_ids": [r.get("forward_pass_id") for r in all_records],
                }
...
try:
    ...
    def generate_with_expert_dist(self, prompts, sampling_params=None, **kwargs):
        """
        ...
        try:
            outputs = _original_generate(self, prompts, sampling_params, **kwargs)
        finally:
            # Auto-stop and dump recording if enabled
            if _config["auto_record"]:
                try:
                    all_data = self.collective_rpc("dump_expert_distribution_record")
                    expert_stats = _extract_expert_stats(all_data, _config["mode"])
...
def get_expert_stats(llm) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if stats is None:
            logger.debug("No expert stats available - recording may not be active or no data accumulated")
